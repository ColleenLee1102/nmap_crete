...
char* read_testcase_in_buff(){
	...
	fp = fopen("/home/zheli/clean_crete/testcase_path.txt","r");
	fgets(path_temp, 255, (FILE*)fp);
	//fscanf(fp,"%s",path);
	printf("read test case path:%s\n",path_temp);
	fclose(fp);

	char * path = strtok(path_temp, "\n");
	puts(path);
	...
	char* buff = (char*)malloc(56);
	memset(buff, 0, 56);

	if(tc.is_test_patch()){
	}else{
	  crete::TestCaseElements elems_ = tc.get_elements();
	  for(uint64_t i = 0; i<elems_.size(); ++i){
		  fprintf(stderr, "elems_[%lu]: ", i);
		  for(uint64_t j = 0; j < elems_[i].data.size(); ++j){
			  fprintf(stderr, "%u ", (uint32_t)elems_[i].data[j]);
			  buff[j] = elems_[i].data[j];
		  }
	  }
	  printf("\n");
   }
...
static int l_receive (lua_State *L)
{
  nsock_pool nsp = get_pool(L);
  nse_nsock_udata *nu = check_nsock_udata(L, 1, true);
  //zl3 check socket must be connected, fake connection
  NSOCK_UDATA_ENSURE_OPEN(L, nu);
